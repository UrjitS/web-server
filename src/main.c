...
char *  get_method (const struct dc_env *env, struct dc_error *err, struct http_packet_info *packet_info, char *raw_data);
char *  get_path (const struct dc_env *env, struct dc_error *err, struct http_packet_info *packet_info, char *raw_data);
...
char *  get_method (const struct dc_env *env, struct dc_error *err, struct http_packet_info *packet_info, char *raw_data) {
    ...
    // GET /x HTTP/1.0
    // We only care about the method and the path for now
...
char *send_file(const struct dc_env *env, struct dc_error *err, __off_t size)
{
    ...
    char * http_time = get_http_time(env, err);
    char str[20] = "";
    snprintf(str, sizeof(str), "%lld", (long long) size);  // Convert the off_t value to a string
//    printf("File size: %ld \n", size);

    // Create packet
    char data[1024] = "";
    dc_strcat(env, data,  "HTTP/1.0 200 OK\r\n");
    dc_strcat(env, data,  http_time);
    dc_strcat(env, data,  "Server: webserver-c\r\n");
...
void send_message_handler(const struct dc_env *env, struct dc_error *err, int client_socket, bool *closed, struct http_packet_info * httpPacketInfo)
{
    ...
    if (httpPacketInfo->read_fd == -1) {
        ...
        printf("FOUND\n");
        // Get file size
        stat(httpPacketInfo->path, &st);
        __off_t size = st.st_size;
        // Send file
        char * data = send_file(env, err, size);
        dc_write_fully(env, err, client_socket, data, dc_strlen(env, data));
        copy(httpPacketInfo->read_fd, client_socket, size);
